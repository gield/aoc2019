...
for permutation in itertools.permutations(range(5, 10)):
    current_output = 0
    solvers = [Solver(memory, [phase_setting])
               for phase_setting in permutation]
    for i in itertools.cycle(range(5)):
        new_input_values = list(solvers[i].input_values) + [current_output]
        solvers[i].input_values = iter(new_input_values)
        current_output = solvers[i].solve()
        if solvers[i].is_finished:
            # We need to get the output of the previous solver
            current_output = solvers[i - 1].output
            break
